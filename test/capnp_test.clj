(ns capnp-test
  "Golden tests for kami.capnp — the Cap'n Proto schema hiccup. They pin numbered fields, List types,
...
(let [{:keys [fail error]} (run-tests 'capnp-test)]
  (when (pos? (+ fail error))
    (throw (ex-info "capnp tests failed" {:fail fail :error error}))))
