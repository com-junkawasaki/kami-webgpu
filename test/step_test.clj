(ns step-test
  "Golden tests for kami.step — the STEP (ISO 10303-21) mechanical-CAD hiccup. They pin entity argument
...
(let [{:keys [fail error]} (run-tests 'step-test)]
  (when (pos? (+ fail error))
    (throw (ex-info "step tests failed" {:fail fail :error error}))))
